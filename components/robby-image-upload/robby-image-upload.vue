<template>
	<view class="imageUploadContainer">
		<view class="imageUploadList">
			<view class="imageItem" v-bind:key="index" v-for="(file,index) in imageListData">
				<image :src="file.path" :class="{'dragging':isDragging(index)}" draggable="true" @tap="previewImage" :data-index="index" @touchstart="start" @touchmove.stop.prevent="move" @touchend="stop"></image>
	...
	export default {
		...
		data() {
			return {
				...
				imageList: [],
				isDestroyed: false,
				showProgress: this.showUploadProgress
			}
		}, 
		mounted: function(){
						...
						if(_self.serverUrl){
							...
							Promise.all(promiseWorkList).then((result)=>{
								...
								for(let i=0; i<result.length;i++){
									result[i].path = http.config.baseUrl+ result[i].qualifiedName
