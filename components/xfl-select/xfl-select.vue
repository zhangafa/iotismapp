<template>
	<div class="show-box" :class="{ disabled: disabled, active: isShowList }" :style="style_Container">
		...
		<!-- 清除按钮图标 -->
		<span v-if="clearable && selectText && selectText != placeholder" class="right-arrow" @click="onClear"><span class="iconfont iconshanchu1 clear"></span></span>

		<!-- 列表框 -->
		<div class="list-container" @click.stop="onListClick" :style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			<span class="popper__arrow"></span>
			<!-- 列表框左上角的空心小三角 -->
			<scroll-view class="list" style="background-color: #fff;" :style="'max-height: ' + listBoxHeight__ + 'em;'" scroll-y="true" scroll-x="true">
				<div
					class="item"
	...
	computed: {
		...
		innerList() {
			...
			orginArr.forEach((val, index) => {
				let value = typeof val === 'object' && this.valuekey in val ? val[this.valuekey] : val;
...
function getNodeInfo(
	...
	const options = { selector, attemptSpaceTime, totalAttemptNum, attemptSpaceRate, id, dataset, rect, size, scrollOffset, properties, computedStyle, context };
