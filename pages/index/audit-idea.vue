<template>
	<uni-popup ref="share" type="bottom" :custom="true">
		<view class="uni-share" :class="isPaddingBottom?'uni-share-padding-bottom':''">
			<view class="uni-share-content" v-if="!isPass">
				<view class="margin-left-sm">意见反馈(非必填)</view>
			...
			<view class="flex">
				...
				<view class="flex-sub margin-sm" v-if="isPass">
					<button class="cu-btn block shadow bg-green" @tap="passClick">提交</button>
				</view>
				<view class="flex-sub margin-sm" v-if="!isPass">
					<button class="cu-btn block shadow bg-red" @tap="unPassClick">关闭</button>
				</view>
			</view>
		</view>
	</uni-popup>
